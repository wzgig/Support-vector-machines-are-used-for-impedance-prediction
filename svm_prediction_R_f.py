"""
Impedance Prediction using SVM (ArcSinh Strategy) - Branch 'f' Specialized
==========================================================================
基于支持向量机（SVM）的阻抗预测模型 - 反双曲正弦变换（ArcSinh Strategy）
专门针对 RL_Series 拓扑中的 R_f 分支进行预测。

Author: GitHub Copilot (Modified based on R_d framework)
Date: 2026-02-14
Description:
    此脚本针对 extracted_RL_Series_Y11_wide.csv 中的 R_f 分支进行 SVR 建模与预测。
    
    问题复盘 (Problem Analysis for Branch 'f'):
    1. 数据分布 (Data Distribution): 
       - R_f 数据特征为负值 (Range: -256 ~ -46)。
       - 数据呈现明显的左偏态 (Left/Negative Skewness, Skew ~ -1.14)。
       - 物理上在此等效电路模型中表现为负阻抗特性 (Active/Fitting Parameter)。
       
    2. 策略选择 (Strategy): 
       - 清洗策略: 强制 R_f < 0，设置合理的负值范围 (如 -5000 ~ -1.0)，排除 0 或 正值异常。
       - 目标变换 (ArcSinh): 对负偏态分布有效。R_f 在 [-260, -40] 区间变换后约为 [-6.2, -4.4]。
         ArcSinh 变换在处理大幅度负值时具有类似对数的压缩效果，能有效拉近长尾数据（Ex: -256）。
       - 特征工程: 同步引入 1/xi, Z_mag 等物理特征。

Usage:
    python svm_prediction_R_f.py
"""
...
class ExperimentConfig:
    ...
    model_save_path: str = "svm_model_ArcSinh_Impedance_f.pkl"
    result_save_path: str = "results_arcsinh_f.txt"
    ...
    def __post_init__(self):
        if self.param_distributions is None:
            self.param_distributions = {
                # 针对 Branch 'f' 优化: 
                # R_f (abs ~ 100-250) 与 R_d (abs ~ 60-800) 量级相近
                # 使用相同的搜索空间
...
class DataProcessor:
    ...
    def load_and_filter(self) -> pd.DataFrame:
        ...
        # --- 自动清洗逻辑 (R_f 专用) ---
        # 现象: R_f 为负值 (Range: -256 ~ -46)。
        # 策略: 移除 0 或正值，允许大负值。
        min_limit = -10000.0 # 宽松下限
        max_limit = -1.0     # 必须为显著负值 (排除 -0.000... 噪声)
        ...
        logger.info(f"R_f 统计: Min={df['R'].min():.2f}, Max={df['R'].max():.2f}, Mean={df['R'].mean():.2f}")
        return df
...
class ImpedanceSVM:
    ...
    def train(self, X_train, y_train):
        ...
        logger.info(f"开始超参数搜索 (Target: ArcSinh(R_f))...")
...
def evaluate_performance(model, X_test, R_test_true, save_path="results_arcsinh_f.txt"):
    """
    评估 (都在物理域 R 进行)
    """
    logger.info("\n=== 模型评估报告 ===")
    ...
    log_msg = f"[Physical Domain R_f] R2: {r2:.4f}\n[Physical Domain R_f] RMSE: {rmse:.4f}\n[Physical Domain R_f] MAE: {mae:.4f}\n[Physical Domain R_f] MAPE: {mape:.2f}%"
    logger.info(log_msg)
