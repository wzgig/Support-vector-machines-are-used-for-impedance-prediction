...
# 输出文件 (.mat)
OUTPUT_MAT_PATH = os.path.join(os.path.dirname(INPUT_CSV_PATH), "predicted_admittance_transfer_function.mat")
# =================================================
...
def generate_matlab_data():
    ...
    for case_name, group in grouped:
        ...
        matlab_data_list.append(case_data)
        
        print(f"{case_name:<20} | {len(poles):<10} | {d_val:.2e}   | {h_val:.2e}")
        # print(f"  Transfer Function Order: {len(den)-1}")

    if not matlab_data_list:
        print("[警告] 未处理任何数据。")
        return

    # 保存为 .mat 文件
    max_len = len(matlab_data_list)
    dt = np.dtype([
        ('case_name', 'O'), 
        ('P', 'd'), ('Q', 'd'), ('V', 'd'), ('xi', 'd'),
        ('poles', 'O'), ('residues', 'O'), ('d', 'd'), ('h', 'd'),
        ('num', 'O'), ('den', 'O') # 使用 Object 类型存储变长数组
    ])
    
    structured_arr = np.zeros((max_len,), dtype=dt)
    
    for i, item in enumerate(matlab_data_list):
        structured_arr[i]['case_name'] = item['case_name']
        structured_arr[i]['P'] = item['P']
        structured_arr[i]['Q'] = item['Q']
        structured_arr[i]['V'] = item['V']
        structured_arr[i]['xi'] = item['xi']
        structured_arr[i]['poles'] = item['poles']
        structured_arr[i]['residues'] = item['residues']
        structured_arr[i]['d'] = item['d']
        structured_arr[i]['h'] = item['h']
        structured_arr[i]['num'] = item['num']
        structured_arr[i]['den'] = item['den']

    sio.savemat(OUTPUT_MAT_PATH, {'admittance_models': structured_arr})
    
    print("-" * 60)
    print(f"成功! 数据已保存至: {OUTPUT_MAT_PATH}")
    print("在 MATLAB 中使用: data = load('predicted_admittance_transfer_function.mat');")
    print("                 models = data.admittance_models;")
    print("绘图示例: tf_sys = tf(models(1).num, models(1).den); bode(tf_sys);")
