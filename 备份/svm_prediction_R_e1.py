    ...
    def __post_init__(self):
        if self.param_distributions is None:
            self.param_distributions = {
                # 策略更新 (2026-02-10 - Branch 'e' 专用): 
                # 1. 之前使用 'scale' 导致 gamma 过小 (因为特征 1/xi 方差极大)，
                #    这使得 RBF 核太宽，"无穷大"的影响泄露到了正常区域 (Bleeding Effect)，
                #    导致 R=1000 的点被预测成 1,000,000。
                #    必须强制使用大 Gamma (>1) 来隔离奇点。
                #    Update: 进一步提高 Gamma，针对极其尖锐的 xi=0 峰值
                'regressor__svr__gamma': [5.0, 10.0, 20.0, 50.0],
                
                # 2. C 值需要适中，太大会过拟合峰值
                #    Update: 配合超级权重，C 不用太大
                'regressor__svr__C': [100, 500, 1000], 
                
                # 3. Epsilon
                'regressor__svr__epsilon': [0.02, 0.05, 0.1],
...
class DataProcessor:
    ...
    def load_and_filter(self) -> pd.DataFrame:
        ...
        # 自动清洗逻辑 (Branch 'e' 禁用)
        # Branch 'e' 正是需要预测极端值的 RL_Series 分支，不能应用简单的 IQR 清洗，
        # 否则会把物理奇点当做噪点删掉。此处显式跳过。
        if self.config.target_branch_id in ['c', 'b', 'd', 'f'] and self.config.target_branch_type == 'RL_Series':
             # 计算分位数
            Q1 = df_filtered['R'].quantile(0.01)
            Q99 = df_filtered['R'].quantile(0.99)
            IQR = Q99 - Q1
            lower_bound = Q1 - 5 * IQR
            upper_bound = Q99 + 5 * IQR
            
            initial_count = len(df_filtered)
            df_filtered = df_filtered[(df_filtered['R'] >= lower_bound) & (df_filtered['R'] <= upper_bound)]
            filtered_count = initial_count - len(df_filtered)
            if filtered_count > 0:
                logger.warning(f"检测到并移除了 {filtered_count} 个极端离群样本 (Threshold: {lower_bound:.2f} ~ {upper_bound:.2f})")
...
class AdmittanceSVM:
    ...
    def train(self, X_train, y_train):
        ...
        # 策略升级 (2026-02-10 - Branch 'e' Extreme Fix):
        # 问题核心: ArcSinh 出现了严重的"饱和效应"。
        # R=10^5 -> y=12.2; R=10^8 -> y=19.1。
        # 在 y 空间误差仅为 7，但在 R 空间误差是 1000倍！
        # 之前的 sqrt(|R|) 权重（300~900）对于这种指数级差异仍然不够惩罚。
        # 新策略: 使用 log10(|R|) 的指数作为权重，或者直接对极端值区域施加阶跃权重。
        
        # 权重设计: 
        # base_weight = log10(1 + |R|)  -> range: 0~6
        # 如果 |R| > 10000 (进入非线性区), 额外乘个系数。
        
        abs_y = np.abs(y_train) # 这里的 y_train 是 R (因为在 TTR fit 之前)
        
        # 基础权重: 对数级增长 (比 sqrt 更温和，防止普通样本完全失效)
        weights = np.log10(1.0 + np.abs(abs_y)) + 1.0 # 1 ~ 7
        
        # 惩罚项: 对于 |R| > 10000 的点，我们不能容忍 y 空间的任何偏差。
        # 赋予它们统治级的权重。
        high_val_mask = np.abs(abs_y) > 10000
        weights[high_val_mask] *= 100.0  # 权重瞬间飙升到 500~700
        
        # 超级极端点 (尖峰): 
        peak_mask = np.abs(abs_y) > 100000
        weights[peak_mask] *= 10.0      # 权重飙升到 5000+
        
        weights = np.clip(weights, 1.0, 10000.0) 
