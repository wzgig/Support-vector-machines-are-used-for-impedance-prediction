"""
Impedance Prediction using SVM (ArcSinh Strategy) - Branch 'b' Specialized
==========================================================================
基于支持向量机（SVM）的阻抗预测模型 - 反双曲正弦变换（ArcSinh Strategy）
专门针对 RL_Series 拓扑中的 R_b 分支进行预测。

Author: GitHub Copilot (Refactored for R_b)
Date: 2026-02-13
Description:
    此脚本实现了一种针对 RL 串联电路中 Branch 'b' 的电阻 R_b 进行预测的模型。
    
    问题复盘 (Problem Analysis for Branch 'b'):
    1. 数据分布 (Data Distribution): 
       - R_b 具有显著的混合符号特征（正负值均存在）。
       - 既然存在极端的奇异值性质 (Singularity)：当 xi 接近 0 时，R_b 会趋向于负无穷大 (例如 -30,000 以上)，
         而在 xi 远离 0 时，R_b 数值较小且平稳 (如 -5 ~ +5)。
       - 这种分布表明 R_b 与 1/xi 高度相关。
       
    2. 策略选择 (Strategy): 
       - 目标变换 (ArcSinh): Essential! 既然 R_b 跨越正负且包含极大的动态范围 (-45000 到 200)，
         ArcSinh 能将线性增长压缩为对数增长，同时无缝处理零点和负数，非常适合此类奇异数据。
       - 特征工程: 必须引入倒数特征 (1/xi)，因为物理上这似乎是一个反比关系 (Singularity near 0)。
       - 鲁棒缩放: 使用 RobustScaler 处理输入特征，抵抗极端值对缩放的影响。

Usage:
    python svm_prediction_R_b.py
"""
...
class ExperimentConfig:
    """实验参数配置类"""
    input_filename: str = "extracted_RL_Series_Y11_wide.csv" # 切换至宽表以获取正确的 R_b 数据
    backup_filenames:Tuple[str, ...] = ("equivalent_circuit_parameters_optimized_accurate_Y11.csv",)
    model_save_path: str = "svm_model_ArcSinh_Impedance_b.pkl"
    ...
    def __post_init__(self):
        if self.param_distributions is None:
            self.param_distributions = {
                # 策略更新 (2026-02-13 - 针对 Branch 'b' 优化): 
                # 1. C 值: 由于存在奇异点（极值），可能需要较大的 C 来拟合这些尖峰，或者较小的 C 来平滑。
                'regressor__svr__C': [100, 1000, 5000, 10000, 20000], 
                
                # 2. Gamma: 数据变化剧烈，gamma 可能需要较高以捕捉局部特征
                'regressor__svr__gamma': ['scale', 0.1, 1.0],
                
                # 3. Epsilon: 在 ArcSinh 变换后，-30000 变为 -11，-5 变为 -2.3。
                # 范围缩小了。Epsilon 应该比较小。
...
class DataProcessor:
    ...
    def load_and_filter(self) -> pd.DataFrame:
        ...
        # 兼容处理: 如果是 wide 表，没有 Branch_ID 列，但有 R_b 列。
        # 如果是 long 表，有 Branch_ID 列。
        if 'R_b' in df.columns and self.config.target_branch_id == 'b':
            # Wide format detected
            logger.info("检测到宽表格式 (Wide Format)，提取 R_b 列...")
            df = df.copy()
            df['R'] = df['R_b'] # 将目标列重命名为 R 以匹配后续逻辑
        ...
        # --- 自动清洗逻辑 (R_b 专用) ---
        # 现象: R_b 在 xi 接近 0 时有巨大的负奇异值。
        # 策略: 根据需求，限制 R 值范围在 [-3000, 3000] 以移除极端的奇异点，使模型聚焦于核心物理区域。
        range_limit = 3000
        mask_range = (df['R'] >= -range_limit) & (df['R'] <= range_limit)
...
class AdmittanceSVM:
    ...
    def train(self, X_train, y_train):
        ...
        logger.info(f"开始超参数搜索 (Target: ArcSinh(R_b))...")
