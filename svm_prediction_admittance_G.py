...
class ExperimentConfig:
    ...
    model_save_path: str = "svm_model_ArcSinh_Impedance.pkl"
    ...
    def __post_init__(self):
        if self.param_distributions is None:
            self.param_distributions = {
                'regressor__svr__C': [100, 500, 1000, 2000, 5000, 10000], # Increased C for better outlier fitting
                'regressor__svr__epsilon': [0.001, 0.01, 0.05, 0.1],      # Tighter epsilon
...
class AdmittanceSVM:
    ...
    def train(self, X_train, y_train):
        ...
        # 4. Calculate Sample Weights (Log-Magnitude)
        weights = 1.0 + np.log10(1.0 + np.abs(y_train))
        
        logger.info(f"开始超参数搜索 (Target: ArcSinh(R))...")
        logger.info(f"应用样本加权 (Sample Weighting): Max Weight={weights.max():.2f}")
